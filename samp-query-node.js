...
class SAMP_Strings {
    ...
    static Decode_String(buffer_data) {
        if (!Buffer.isBuffer(buffer_data)) return '';
        
        return Array.from(buffer_data)
            .map(byte => byte < this.charset.length ? this.charset[byte] : '?')
            .join('')
...
class samp_query {
    ...
    async Measure_Server_Latency(server_config) {
        return new Promise((resolve) => {
            ...
            const Send_Ping = () => {
                if (is_socket_closed) return null;
            ...
            const Update_Timeout = () => {
                ...
                timeout_id = setTimeout(() => {
                    Close_Socket();
                    const avg_latency = ping_results.length > 0 
    ...
    Parse_Server_Info(server_data) {
        if (!server_data || server_data.length < 11) return null;
    ...
    Parse_Server_Rules(rules_data) {
        if (!rules_data || rules_data.length < 2) return {};
    ...
    Process_Rules(raw_rules) {
        const processed_rules = Object.fromEntries(
            Object.entries(raw_rules)
    ...
    Parse_Player_List(player_data) {
        if (!player_data || player_data.length < 2) return [];
    ...
    async Get_Server_Info(server_options) {
        const server_config = typeof server_options === 'string' 
            ? { host: server_options, port: Server_Settings.DEFAULT_PORT, timeout: Network_Config.TIMEOUT }
            : { 
        ...
        try {
            ...
            const players_promise = server_info.onlinePlayers <= Server_Settings.MAX_PLAYERS
                ? this.Query_Server(server_config, Query_Types.PLAYERS, 1500)
...
module.exports = function(server_options, callback) {
    Global_SAMP_Client.Get_Server_Info(server_options)
